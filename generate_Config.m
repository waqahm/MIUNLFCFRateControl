...
% ------- Writing Quality Levels File --------
fileID_QL = fopen(strcat(sprintf('%s\\',S.pathCFG),S.QualityLevelFileName),'w');

for qi=1:size(S.Quality_Levels,1)
    for qj=1:size(S.Quality_Levels,2)
        fprintf(fileID_QL,'%d       ',S.Quality_Levels(qi,qj));  
    end
    fprintf(fileID_QL,'\n'); 
end
fclose(fileID_QL);
...
fprintf(fileID,'ProposedMethod                : 1           #Enable(1) or Disable(0) Quality Baseline Compression   \n');
fprintf(fileID,'CFMode                        : 0              \n');
%======== File I/O =====================
...
fprintf(fileID,'\n#============ Rate Control ======================\n');
fprintf(fileID,'RateControl                         : %d                # Rate control: enable rate control\n',S.RateControl);
fprintf(fileID,'TargetBitrate                       : 1000000          # Rate control: target bitrate, in bps\n');
fprintf(fileID,'KeepHierarchicalBit                 : 0                # Rate control: keep hierarchical bit allocation in rate control algorithm\n');
fprintf(fileID,'LCULevelRateControl                 : 0                # Rate control: 1: LCU level RC; 0: picture level RC\n');
fprintf(fileID,'RCLCUSeparateModel                  : 0                # Rate control: use LCU level separate R-lambda model\n');
