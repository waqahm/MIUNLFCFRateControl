...
if (DATASET==1) % Handling Lytro Dataset
    ...
    for k=1:4 % size(IMG,2)
        ...
        path_output=sprintf('%s\\Lytro_13\\%s',P.pathMPVS_Sequences,Images{k});
...
elseif (DATASET==4)  % Handling HCI Dataset
    W=512;
    H=512;
    inputBPP=10;
    outputBPP=8;
    for input=1:2
        path_input=sprintf('%s\\HCI\\%d\\',P.PathDataset,input);
        path_output=sprintf('%s\\HCI\\%d',P.pathMPVS_Sequences,input);
        
        if exist(path_input, 'dir')% check if input LF exists
            mkdir(path_output)
            [Status]=generate_MPVS_From_LF(path_input, path_output,P.NumberOfLayers,P.NumberOfFrames,W,H,SAMP,inputBPP,outputBPP,DATASET);
        else
            msgbox('input LF "/Fraunhofer_IIS/1/" does not exist','Directory Error','error');
            error('input LF "/Fraunhofer_IIS/1/" does not exist')
        end
    end
