...
elseif(DATASET==2)
    QP_List=[17825 89128 356515 1782579 13369344];
...
%         path_input_db=sprintf('%s\\Stanford\\%s\\',P.PathDataset,'tarot');
        ...
        path_input=sprintf('%s\\Stanford\\Output_%s\\QP_%d\\',P.pathEncodingSystem,'Bunny',QP_List(q));        
        path_logFile=sprintf('%s\\Stanford\\Output_%s\\',P.pathEncodingSystem,'Bunny');
        
        path_output=sprintf('%s\\Stanford\\8bpp\\%s_dec_%d',P.pathCompressedMats,'Bunny',QP_List(q));
        
        path_input_db=sprintf('%s\\Stanford\\%s\\',P.PathDataset,'Bunny')
...
elseif(DATASET==3)
 ...
 elseif(DATASET==4)
     ...
     QP_List=[2359 11796 47185 235929 1769472];
    
    for sub=1:2
     
    for q=1:size(QP_List,2) % size(IMG,2)
        path_input=sprintf('%s\\HCI\\Output_%d\\QP_%d\\',P.pathEncodingSystem,sub,QP_List(q));
        path_logFile=sprintf('%s\\HCI\\Output_%d\\',P.pathEncodingSystem,sub);
        path_output=sprintf('%s\\HCI\\%d_dec_%d',P.pathCompressedMats,sub,QP_List(q));
        path_input_db=sprintf('%s\\HCI\\%d\\',P.PathDataset,sub);
        if exist(path_input,'dir')
            mkdir(sprintf('%s\\HCI\\',P.pathCompressedMats))
            res=mvs2mat(path_input,path_output,512,512,P.NumberOfLayers,P.NumberOfFrames,P.POCArray,P.VIDArray,DATASET,QP_List(q),path_logFile,path_input_db,P.Write_PPM);
        else
            fprintf('output encoded LF "Fraunhofer_IIS/Output_%s/QP_%d/" does not exist. \n','Set2',QP_List(q))
        end
    end   
