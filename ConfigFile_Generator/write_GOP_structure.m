    ...
    for poc=1:GridSizePOC
        ...
        if(Vid==1)
            ...
            if(poc == GridSizePOC && rem(S.NumberOfFrames,2))
                A{poc+1,:}=sprintf('Frame%d:      B    %2d    %2d  0  0       %.4f   0        0        0       %2d      %2d     %*s    %d    %3s   %3s   %*s',poc+1,poc+1,QpMat(1,1),Offset(poc),NumberOfRefPics(1)+NoOfActiveRefLayerPics(1),NumberOfRefPics(1),max(NumberOfRefPics)*3,reference_pictures_POC{1,:},predict(1),' ',' ',max(cellfun('size',reference_idcs,2))*3,num2str(reference_idcs{1,:}));
            end
            ...
            if(Vid<11)
                ...
                %add extra dummy frame to make GOP even if LF is odd size
                if(poc == GridSizePOC && rem(S.NumberOfFrames,2))
                    A{poc+1,:}=sprintf('Frame%d_l%d:   B    %2d    %2d  0  0       %.4f   0        0        0       %2d      %2d     %*s    %d    %3s   %3s   %*s',poc+1,Vid-1,poc+1,QpMat(1,1),Offset(poc),NumberOfRefPics(1)+NoOfActiveRefLayerPics(1),NumberOfRefPics(1),max(NumberOfRefPics)*3,reference_pictures_POC{1,:},predict(1),' ',' ',max(cellfun('size',reference_idcs,2))*3,num2str(reference_idcs{1,:}));
                end
