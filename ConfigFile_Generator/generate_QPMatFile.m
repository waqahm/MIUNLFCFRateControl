...
for IdxPoc=1:GridSizePOC
    for IdxVid=1:GridSizeVID
        ...
        Quality_Levels_Grid(IdxPoc,IdxVid)=WPOC(IdxPoc)+WVID(IdxVid);
...
for Id=1:size(POCArray,2)
    ...
    Quality_Levels_Updated(Id,:)=Quality_Levels_Grid(POCArray(Id)+1,:);
end

for Id=1:size(VIdArray,2)
    QpMat(:,Id)=FQP_Updated(:,VIdArray(Id)+1);
    Quality_Levels(:,Id)=Quality_Levels_Updated(:,VIdArray(Id)+1);
end

P(:).QpMat=QpMat;
P(:).Quality_Levels=Quality_Levels;
